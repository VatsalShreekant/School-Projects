function [a,accuracy] = question4b(trainingData)
% Return Value:
...
train_Veri = veri_Data(1:35,:);
test_Veri = veri_Data(36:50,:);
...
trainBC = [train_Veri; trainVirginia];
...
%% Question 2(Modified for Q4): Use the 30% set (test samples) and the weight vector 
% computed in the previous step to calculate the classification accuracy 
% of the classifier.
testingBC = [test_Veri; testVirginia];
testingBCTransposed = testingBC';
x2x3w2Test = testingBCTransposed(1:2,find(testingBCTransposed(3,:)==2));
[x2x3w2Rows x2x3w2Cols] = size(x2x3w2Test);
rowOfOnesw1Test = ones(1,x2x3w2Cols);
 
% Normalize features for class 3 labeled values.
x2x3w3Test = testingBCTransposed(1:2,find(testingBCTransposed(3,:)==3))*(-1);
[x2x3w3Rows x2x3w3Cols] = size(x2x3w3Test);
...
x2x3Test = horzcat(x2x3w2Test,x2x3w3Test);
